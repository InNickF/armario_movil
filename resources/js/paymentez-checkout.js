      ...
      */
      ...
      if (response.transaction) {
        ...
        if (response.transaction.status == 'failure') {
          document.getElementById('response').innerHTML = response.transaction.message ? response.transaction.message : 'Error desconocido en servicio de pago';
          return;
        }
        ...
        axios.post(process.env.MIX_API_URL + '/orders/pay/' + orderId, {
            orderUuid: orderUuid,
            payment: response,
          })
          .then(function (response) {
            ...
            setTimeout(function () {
              // Redirect to bill 
              window.location = process.env.MIX_APP_URL + '/bill/' + orderId
            }, 3000);


          })
          .catch(function (error) {
            document.getElementById('response').innerHTML = error.message ? error.message : 'Error desconocido al procesar el pedido';
          });
