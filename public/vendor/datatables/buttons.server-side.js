(function ($, DataTable) {
    ...
    //         console.log(data);
    //         return data.role[0] === 'super-user';
    //     }
    // );

    // DataTable.ext.buttons.filterByRole = {
    //     className: 'buttons-role',
  
    //     text: function (dt) {
    //         return dt.i18n('buttons.role', 'Role');
    //     },

    //     action: function (e, dt, button, config) {
    //         var url = [location.protocol, '//', location.host, location.pathname].join('');
    //         window.location = url + '?role=' + e.target.value;
    //     }
    // };


    DataTable.ext.buttons.filterByUser= {
        className: 'buttons-role',

        text: function (dt) {
            return dt.i18n('buttons.user', 'User');
        },

        action: function (e, dt, button, config) {
            var url = [location.protocol, '//', location.host, location.pathname].join('');
            window.location = url + '?userId=' + e.target.value;
        }
    };
