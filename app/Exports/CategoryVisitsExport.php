...
class CategoryVisitsExport implements FromCollection, WithHeadings
{
    ...
    {
        $rawData = Auth::user()->store->getVisitsBySubcategories($this->start, $this->end);

        $data = new Collection();
        $rawData->each(function($catItemsByMonth, $catName) use (&$data) {
           $row = [$catName];
           $row = array_merge($row, $catItemsByMonth->flatten()->toArray());
           $data[] = $row;
        });
        return $data;
    }
